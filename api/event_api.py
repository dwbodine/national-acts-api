...
def get_events_and_orders_secured():
    ...
    if request.args.get("portal") is not None:
        is_portal = get_override_bool_value_or_default(request.args.get("portal"))
...
@event_api.route("/events/setEventsLiveInBandsInTown", methods=["POST"])
@jwt_required()
def set_event_bands_in_town_secured():
    """
    API method to mark event(s) as live in BandsInTown
    """
    event_ids: list[int] = request.json.get("eventIdList", [])
    ...
    service = EventService()
    result = service.mark_events_live_in_bands_in_town(event_ids)
    if result is False:
        return {"msg": "Internal Server Error"}, 500
    return convert_to_json(result)

...
from common.utility import (
    ...
    get_pacific_purchase_timestamp_from_order,
)
...
class DataRefreshService:
    ...
    def refresh_database_from_ticket_socket(
        ...
        try:
                ...
                for evt in all_events:
                    ...
                    if existing_event:
                        ...
                        if ex_row:
                            ...
                            event_data["id"] = ticket_socket_event_id
                            event_success = event_service.add_to_external_events(
                                event_data, evt, cnx
                            )
                    else:
                        event_add_new = True
                    ...
                    if ticket_socket_event_id and len(evt.ticket_types) > 0:
                        ...
                        for ticket_type in evt.ticket_types:
                            ...
                            ticket_type_data = {
                                ...
                                "order": get_override_int_value_or_default(
                                    ticket_type.ticket_type_order, default=1
                                ),
                            }
                            ...
                            if existing_ticket_type:
                                ...
                                sql = """INSERT INTO TicketSocketTicketTypes
                                        (TicketSocketTicketTypeId, TicketSocketEventId,
                                            TicketTypeName, TotalAvailable, IsActive, TicketTypeOrder, LastUpdate)
                                                VALUES (%(ticketSocketTicketTypeId)s,
                                                %(ticket_socket_event_id)s, %(ticketTypeName)s,
                                                %(totalAvailable)s, %(is_active)s, %(order)s,
                                                CURRENT_TIMESTAMP)"""
                                ticket_socket_type_id = db_insert(
                                    sql, ticket_type_data, cnx
                                )
                                ticket_type_success = (
                                    ticket_socket_type_id is not None
                                    and ticket_socket_type_id > 0
                                )
                    ...
                    if ticket_socket_event_id and len(evt.orders) > 0:
                        ...
                        for order in evt.orders:
                            ...
                            purchase_date = get_pacific_purchase_date_from_order(order)
                            purchase_timestamp = (
                                get_pacific_purchase_timestamp_from_order(order)
                            )
                            ...
                            order_data = {
                                ...
                                "purchaseUnixTimestamp": get_override_int_value_or_default(
                                    order.purchase_unix_timestamp
                                ),
                            ...
                            if existing_order:
                                ...
                                sql = """UPDATE TicketSocketOrders
                                        ...
                                        PurchaseUnixTimestamp=%(purchaseUnixTimestamp)s,
                                ...
                                sql = """INSERT INTO TicketSocketOrders
                                            ...
                                            PurchaseDate, PurchaseTimestamp, PurchaseUnixTimestamp, 
                                            Phone, PhoneFormatted, EventId, UserId,
                                    ...
                                    %(purchaseDate)s, %(purchaseTimestamp)s, %(purchaseUnixTimestamp)s,
                                    %(phone)s, %(phone_formatted)s, 
            ...
            if cnx is not None and cnx.open:
                cnx.close()
        ...
        logger.info(
            "refresh_database_from_ticket_socket - completed, results: %s", results
        )

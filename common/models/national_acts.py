...
class SellerEventCategory:
    ...
    is_visible_on_portal: bool = True
    has_events: bool = False
    seller_rate_percent: float = 0.0
    ...
    def __populate_from_seller_id_and_ticket_socket_id(
        ...
        if sec:
            ...
            self.is_visible_on_site = get_override_bool_value_or_default(
                sec["IsVisibleOnSite"]
            )
            self.is_visible_on_portal = get_override_bool_value_or_default(
                sec["IsVisibleOnPortal"]
            )
            self.seller_rate_percent = get_override_float_value_or_default(
                sec["SellerRatePercent"]
            )
...
class VipEvent(TicketSocketEvent):
    ...
    total_service_fees: float = 0
    total_service_fees_usd: float = 0
    total_tickets: int = 0
    ...
    def get_totals(self):
        ...
        total_revenue_refunded_usd: float = 0
        total_revenue_charged_back: float = 0
        total_revenue_charged_back_usd: float = 0
        total_service_fee_revenue_refunded: float = 0
        total_service_fee_revenue_refunded_usd: float = 0
        total_service_fee_revenue_charged_back: float = 0
        total_service_fee_revenue_charged_back_usd: float = 0
        total_checked_in: int = 0
        shirtd: dict = {}
        for order in self.orders:
            ...
            if order.has_refunds is True:
                ...
                total_revenue_refunded += order.revenue_refunded
                total_revenue_refunded_usd += order.revenue_refunded_usd
                total_service_fee_revenue_refunded += order.service_fee_revenue_refunded
                total_service_fee_revenue_refunded_usd += (
            ...
            if order.has_chargebacks is True:
                ...
                total_revenue_charged_back += order.revenue_charged_back
                total_revenue_charged_back_usd += order.revenue_charged_back_usd
                total_service_fee_revenue_charged_back += (
                    order.service_fee_revenue_charged_back
                )
                total_service_fee_revenue_charged_back_usd += (
            ...
            if order.is_deleted is not True:
                if order.is_comped is not True:
                    total_revenue += order.revenue
                    total_revenue_usd += order.revenue_usd
                    total_service_fees += order.service_fees
                    total_service_fees_usd += order.service_fees_usd
                    total_tickets += order.num_tickets

                if len(order.tickets) > 0:
                    for ticket in order.tickets:
                        if (
                            order.is_comped is True
                            and ticket.attendee_phone is not None
                            and len(ticket.attendee_phone) > 0
                        ):
                            self.has_phone_data = True
        ...
        self.total_service_fees_usd = total_service_fees_usd
        ...
        self.revenue_refunded_usd = total_revenue_refunded_usd
        self.revenue_charged_back = total_revenue_charged_back
        self.revenue_charged_back_usd = total_revenue_charged_back_usd
        self.service_fee_revenue_refunded = total_service_fee_revenue_refunded
        self.service_fee_revenue_refunded_usd = total_service_fee_revenue_refunded_usd

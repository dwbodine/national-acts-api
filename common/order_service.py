...
class OrderService:
    ...
    def get_orders(
        ...
        if start is not None:
            midnight_start = get_pacific_date_from_unix_timestamp(start)

        midnight_end: str = None
        if end is not None:
            end_str = get_pacific_date_from_unix_timestamp(end)
        ...
        if ts_order_id is not None and ts_order_id > 0:
            ...
            if search_term.isnumeric():
                ...
                search_sql = """CONCAT_WS (' ', Sellers.Name,
            ...
            if ignore_flags is not True:
                ...
                if not show_hidden_from_dashboard:
                    where_clause.append("ExternalEvents.ExcludeFromDashboard <> 1")
        ...
        for row in order_rows:
            ...
            order.last_update = get_override_string_value_or_default(row["LastUpdate"])
            ...
            order.purchase_unix_timestamp = get_override_int_value_or_default(
                row["PurchaseUnixTimestamp"]
            )
            ...
            if multiplier > 0:
                order.exchange_rate = order.exchange_rate * multiplier
    ...
    def get_tickets_from_order_id(
        ...
        sql = """SELECT TicketSocketOrderTickets.*,
                    COALESCE(TicketSocketTicketTypes.TicketTypeOrder, 1) AS TicketTypeOrder
                FROM TicketSocketOrderTickets
                JOIN TicketSocketOrders on TicketSocketOrderTickets.TicketSocketOrderId = TicketSocketOrders.Id
                LEFT JOIN TicketSocketTicketTypes ON 
                    TicketSocketOrderTickets.TicketSocketTicketTypeId = TicketSocketTicketTypes.TicketSocketTicketTypeId
	                AND TicketSocketTicketTypes.TicketSocketEventId = TicketSocketOrders.TicketSocketEventId
                WHERE TicketSocketOrderTickets.TicketSocketOrderId =%(ticket_socket_order_id)s"""
        if ignore_flags is not True:
            sql += """ AND TicketSocketOrderTickets.IsActive=1"""
        ...
        for row in rows:
            ...
            if is_refunded is True:
                ticket.refund_date = get_override_string_value_or_default(
                    row["RefundDate"]
                )

            is_checked_in = get_override_bool_value_or_default(row["IsCheckedIn"])
            ticket.is_checked_in = is_checked_in
            checked_in_date = get_override_string_value_or_default(row["CheckedInDate"])
            if is_checked_in is True and checked_in_date is not None:
                ticket.checked_in_date = get_pacific_date_from_utc_datetime_string(
                    checked_in_date
                )
            ...
            if is_charged_back is True:
                ticket.chargeback_date = get_override_string_value_or_default(
                    row["ChargebackDate"]
                )

            ticket.ticket_type_order = get_override_int_value_or_default(
                row["TicketTypeOrder"], default=1
            )

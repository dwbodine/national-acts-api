...
class SenderApiService:
    ...
    def create_subscriber(self, subscriber: Subscriber):
        ...
        response = requests.request(
            "POST", url, headers=headers, json=payload, timeout=300
        )
        ...
        if success is False:
            ...
            if err_msg is not None:
                fail_msg += " - " + err_msg
                if (
                    err_msg.lower().find("phone") >= 0
                ):  # invalid phone, clear out and try again
                    self.clear_subscriber_phone(subscriber.order_id)
                elif (
                    err_msg.lower().find("email") >= 0
                ):  # invalid email, cannot be a subscriber
    ...
    def update_subscriber(self, subscriber: Subscriber):
        ...
        response = requests.request(
            "PATCH", url, headers=headers, json=payload, timeout=300
        )
        response_json = response.json()
    ...
    def get_subscribers_csv(self, stored_subscribers: list[Subscriber]):
        ...
        """
        with open("subscribers.csv", "w", encoding="utf-8") as f:
            csv: str = (
            ...
            if len(stored_subscribers) > 0:
                for s in stored_subscribers:
                    ...
                    csv += (
                        f'"{purchaser_zip}","{venue}","{venue_address}","{venue_city}",'
                    )
